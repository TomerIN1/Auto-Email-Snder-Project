...
os.environ['SNOWFLAKE_ACCOUNT'] = 'lumos.eu-west-1'
os.environ['SNOWFLAKE_USER'] = 'TABLEAU_USER'
os.environ['SNOWFLAKE_PASSWORD']='Kuku2019'
os.environ['SNOWFLAKE_DATABASE'] = 'DATALAKE'
os.environ['SNOWFLAKE_DATAWAREHOUSE'] = 'LOAD_WH'

#Email environment variables
os.environ['EMAIL_SMTP_SERVER'] = 'smtp.gmail.com'
os.environ['EMAIL_SMTP_PORT'] = '465'
os.environ['EMAIL_SENDER'] = 'binotification@lumos-global.com'
os.environ['EMAIL_PASSWORD'] = 'N0valum05!233'
...
def flag_query():
    return '''
           ...
           FROM DWH_NL.CONTRACT_DAILY
           '''

def main_query(target):
    if target == 'SELLER':
        dynamic_text = "CASE WHEN CONSECUTIVE_NO_ACTIVITY_DAYS <= 2 THEN 'tomer.itzhakov.nevo@lumos-global.com' ELSE 'tomerikoka@gmail.com' END AS EMAIL"
    elif target == 'TSM':
        dynamic_text = "TSM_TSO_EMAIL_XXX AS EMAIL"
    elif target == 'RM':
        dynamic_text = "TSM_TSO_NAME_XXX, RM_EMAIL_XXX AS EMAIL"
    else:
    ...
    return '''
        ...
        SELECT A.CONTRACT_SHORT_NUM,
               ...
               C.MANAGER_NAME,
               C.MANAGER_EMAIL
        FROM DWH_NL.CONTRACT_DAILY A
        LEFT JOIN DWH_NL.CONTRACT B
        ON A.CONTRACT_KEY = B.CONTRACT_KEY
        LEFT JOIN DWH_NL.VW_CRM_USERS_SALES_MANAGEMENT_HIERARCHY C
        ON B.CONTRACT_CREATED_BY_USER_KEY = C.USER_KEY
        LEFT JOIN DWH_NL.CONTACT D
        ON B.MAIN_PAYER_CONTACT_KEY = D.CONTACT_KEY
        LEFT JOIN DWH_NL.VW_SERIAL_ID E
        ON A.SPS_SHORT_NUM = E.SERIAL_NUMBER
        WHERE A.CONTRACT_ACTIVITY_STATUS IN ('Active', 'Churn')
        AND B.CONTRACT_MODEL_TYPE = 'Lease To Own'
        AND DATE(A.CONTRACT_CREATION_DATE) >= CURRENT_DATE()-732
        AND A.LAST_PAYMENT_DATE IS NOT NULL
        AND B.CONTRACT_GENERATION = 'Unified'
        AND A.PARTNER_KEY = '001b000000HYu2bAAD'
...
def mail_sender():
    ...
    if data_relevant:
        ...
        df_daily_status = pd.read_sql_query(main_query('SELLER'),engine)
        send_to_group()

        df_daily_status = pd.read_sql_query(main_query('TSM'),engine)
        send_to_group()

        df_daily_status = pd.read_sql_query(main_query('RM'),engine)
        send_to_group()
